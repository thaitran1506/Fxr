...
only 1002 genes have entrez_ID

Read in the read count table to get the universal background gene
```{r}
setwd("~/Desktop/FXR/Fxr/Data")
data = na.omit(read.csv("gene count.csv", header = TRUE, stringsAsFactors = FALSE, sep = ",", row.names = 1))
head(data)
```
...
```{r}
gene_universe = ensembl_entrezID(gene_universe, keys = row.names(gene_universe))

```


```{r}
gene_count = data[gene_universe$X, ]
...
```{r}
write.csv(gene_count, "gene count.csv")
```
